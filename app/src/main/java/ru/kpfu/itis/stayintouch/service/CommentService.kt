...
interface CommentService {
    ...
    fun createComment(@Header("Authorization") token: String,
                      ...
                      @Body comment: Comment) : Single<Comment>

    @POST("/api/comments/{id}/add-answer")
    fun createAnswer(@Header("Authorization") token: String,
                     @Path("id") commentId: String,
                     @Body text: Comment) : Single<Comment>

    @GET("api/news/{postId}/comments")
    fun getCommentsByPostId(@Path("postId") postId: Int,
                            @Query("offset") offset: Int) : Single<List<Comment>>
    ...
    @GET("/api/comments/{id}")
    fun getCommentById(@Header("Authorization") token: String,
                       @Path("id") commentId: String) : Single<Comment>
}